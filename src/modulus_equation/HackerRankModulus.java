...
public class HackerRankModulus {
	...
	public static void main(String[] args) {
		...
		//Set iterations = 1 so the program runs once
		//double iterations = myScan.nextDouble();
		double iterations = 1;
	...
	/*	Alternate Method: A^-B mod X <==> (A^(b+1))*(A^-1) mod X		
			...
			System.out.println("calculating mod of " + A + " to power " + (B-1) + " mod " + X);
			System.out.println("Answer is: "+ positiveModulus(A, Math.abs(B-1), X));
			System.out.println("calculating inverse of " + A + " mod" + X);
			System.out.println("Answer is: "+ AInverseModX(A, X));				
				double mod = positiveModulus(A, Math.abs(B-1), X);
				double inverse = AInverseModX(A, X);
				double answer = (mod*inverse) % X; 
				System.out.println((long)answer);
			}
	*/
		}	
	...
	public static double positiveModulus(double A, long B, double X){
		...
		// create an ArrayList with numbers that correspond to 1's in the binary array 
